...
df_model = df[df["Center"] == "EHBH"].copy()
...
convars = ["Age", "BMI", "AFP", "ICG", "TP0", "ALB0", "PALB0", "PLT0", "INR0",
           ...
           "GGT1", "ALP1", "CR1", "TBI1", "Liver GATA3", "Liver RAMP2", "LRGR"]
...
y_prob = mlp_model.predict_proba(X_scaled)[:, 1]
...
print(f"\n===== Modeling Group (Intraoperative - MLP) =====")
...
y_pred_external = mlp_model.predict(X_external_scaled)
y_prob_external = mlp_model.predict_proba(X_external_scaled)[:, 1]
...
print(f"\n===== External Validation (Intraoperative - MLP) =====")
...
try:
    explainer = shap.Explainer(lambda x: mlp_model.predict_proba(x)[:, 1], X_external_scaled)
    shap_values = explainer(X_external_scaled)

    shap.summary_plot(shap_values, X_external, show=False)
    plt.title(f"SHAP Summary (External - {PHASE}, MLP)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title(f"SHAP Feature Importance (Bar, External - {PHASE}, MLP)")
    plt.tight_layout()
    plt.show()
