...
model_path = os.path.join(MODEL_DIR, f"ENlog_{PHASE}.pkl")
le_path = os.path.join(MODEL_DIR, f"ENlog_label_encoders_{PHASE}.pkl")
feat_path = os.path.join(MODEL_DIR, f"ENlog_feature_list_{PHASE}.pkl")
...
print("\n===== External Validation (Postoperative - ENlog) =====")
...
try:
    ...
    plt.title("SHAP Summary (External - Postoperative, ENlog)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title("SHAP Feature Importance (Bar, External - Postoperative, ENlog)")
    plt.tight_layout()
    plt.show()
