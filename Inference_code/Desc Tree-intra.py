...
model_path = os.path.join(MODEL_DIR, f"DecisionTree_{PHASE}.pkl")
le_path = os.path.join(MODEL_DIR, f"Decitree_label_encoders_{PHASE}.pkl")
feat_path = os.path.join(MODEL_DIR, f"Decitree_feature_list_{PHASE}.pkl")
...
print("\n===== External Validation (Intraoperative) =====")
...
try:
    ...
    plt.title("SHAP Summary (External - Postoperative)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title("SHAP Feature Importance (Bar, External - Postoperative)")
    plt.tight_layout()
    plt.show()
