...
print("\n===== External Validation (Postoperative - ExtraTrees) =====")
...
try:
    ...
    plt.title("SHAP Summary (External - Postoperative, ExtraTrees)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title("SHAP Feature Importance (Bar, External - Postoperative, ExtraTrees)")
    plt.tight_layout()
    plt.show()
