...
model_path = os.path.join(MODEL_DIR, f"ExtraTrees_{PHASE}.pkl")
le_path = os.path.join(MODEL_DIR, f"ExtraTrees_label_encoders_{PHASE}.pkl")
feat_path = os.path.join(MODEL_DIR, f"ExtraTrees_feature_list_{PHASE}.pkl")
...
print("\n===== External Validation (Intraoperative - ExtraTrees) =====")
...
try:
    ...
    plt.title("SHAP Summary (External - Intraoperative, ExtraTrees)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title("SHAP Feature Importance (Bar, External - Intraoperative, ExtraTrees)")
    plt.tight_layout()
    plt.show()
