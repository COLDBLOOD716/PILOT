...
model_path = os.path.join(MODEL_DIR, f"GBM_{PHASE}.pkl")
le_path = os.path.join(MODEL_DIR, f"GBM_label_encoders_{PHASE}.pkl")
feat_path = os.path.join(MODEL_DIR, f"GBM_feature_list_{PHASE}.pkl")
...
print("\n===== External Validation (Intraoperative - GBM) =====")
...
try:
    ...
    plt.title("SHAP Summary (External - Intraoperative, GBM)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title("SHAP Feature Importance (Bar, External - Intraoperative, GBM)")
    plt.tight_layout()
    plt.show()
