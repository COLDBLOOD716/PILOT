...
y_prob_external = svm_model.predict_proba(X_external_scaled)[:, 1]
...
try:
    explainer = shap.Explainer(lambda x: svm_model.predict_proba(x)[:, 1], X_external_scaled)
    shap_values = explainer(X_external_scaled)

    shap.summary_plot(shap_values, X_external, show=False)
    plt.title(f"SHAP Summary (External - {PHASE}, SVM)")
    plt.tight_layout()
    plt.show()

    shap.summary_plot(shap_values, X_external, plot_type="bar", show=False)
    plt.title(f"SHAP Feature Importance (Bar, External - {PHASE}, SVM)")
    plt.tight_layout()
    plt.show()
