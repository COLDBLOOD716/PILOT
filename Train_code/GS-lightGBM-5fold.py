...
def complexity_lgb(params):
    depth = params.get("max_depth", -1)
    depth_val = 0 if depth==-1 else depth
    return depth_val + params.get("num_leaves",31) + params.get("n_estimators",100)
...
for var_set_name, var_set in variable_sets.items():
    ...
    joblib.dump(label_encoders, os.path.join(OUTPUT_DIR, f"LGB_label_encoders_{var_set_name}.pkl"))

    feature_list = var_set["catvars"] + var_set["convars"]
    joblib.dump(feature_list, os.path.join(OUTPUT_DIR, f"LGB_feature_list_{var_set_name}.pkl"))

    X = df[feature_list].reset_index(drop=True)
    y = df[TARGET].reset_index(drop=True)

    sample_weights_whole = np.where(y==1, 4.0, 1.0)
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(outer_kf.split(X, y), start=1):
            ...
            sample_weights_fold = sample_weights_whole[train_idx]
            ...
            gs.fit(X_train, y_train, sample_weight=sample_weights_fold)
            ...
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_lgb(params_list[i]))
            chosen_idx = candidate_idxs_sorted[0]
            chosen_params = params_list[chosen_idx]

            best_model = lgb.LGBMClassifier(random_state=seed, **chosen_params)
            best_model.fit(X_train, y_train, sample_weight=sample_weights_fold)
    ...
    all_models_sorted = sorted(all_models, key=lambda x:(-x[3], complexity_lgb(x[1])))
    final_model = all_models_sorted[0][0]
    joblib.dump(final_model, os.path.join(OUTPUT_DIR, f"LGB_{var_set_name}.pkl"))
    print(f"Saved best model for {var_set_name}")

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-LightGBM.xlsx")
