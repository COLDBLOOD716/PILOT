...
def complexity_mlp(params):
    h = params.get("mlp__hidden_layer_sizes", (100,))
    total_units = sum(h if isinstance(h, (list, tuple)) else [h])
    alpha = params.get("mlp__alpha", 1e-4)
    return total_units + 0.0 * np.log10(alpha + 1e-12)
...
for var_set_name, var_set in variable_sets.items():
    ...
    le_filename = os.path.join(OUTPUT_DIR, f"MLP_label_encoders_{var_set_name}.pkl")
    ...
    feat_filename = os.path.join(OUTPUT_DIR, f"MLP_feature_list_{var_set_name}.pkl")
    joblib.dump(feature_list, feat_filename)
    print(f"Saved feature list: {feat_filename}")

    X_all = df_local[feature_list].reset_index(drop=True)
    y_all = df_local[TARGET].reset_index(drop=True)
    all_seed_results = []
    all_models = []
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(outer_kf.split(X_all, y_all), start=1):
            ...
            cont_cols = [c for c in var_set["convars"] if c in X_train_raw.columns]
            ...
            threshold = best_mean - float(stds[best_idx])

            candidate_idxs = [i for i, m in enumerate(means) if m >= threshold]
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_mlp(params_list[i]))
            ...
            final_pipeline.set_params(**chosen_params)
            ...
            try:
                full_auc = roc_auc_score(y_all, final_pipeline.predict_proba(X_all)[:, 1])
            except Exception:
                full_auc = -np.inf
            all_models.append((final_pipeline, chosen_params, seed, full_auc))
    ...
    all_models_sorted = sorted(all_models, key=lambda x: (-x[3], complexity_mlp(x[1])))
    final_model = all_models_sorted[0][0]

    model_filename = os.path.join(OUTPUT_DIR, f"MLP_{var_set_name}.pkl")
    joblib.dump(final_model, model_filename)
    print(f"Saved best model: {model_filename}")

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-MLP.xlsx")
