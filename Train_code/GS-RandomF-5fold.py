...
def complexity_rf(params):
    md = params.get("max_depth", 1) if params.get("max_depth") else 20
    n_est = params.get("n_estimators", 50)
    mss = params.get("min_samples_split", 2)
    msl = params.get("min_samples_leaf", 1)
    return md * n_est * (mss + msl)
...
for var_set_name, var_set in variable_sets.items():
    ...
    le_filename = os.path.join(OUTPUT_DIR, f"RandomForest_label_encoders_{var_set_name}.pkl")
    ...
    feat_filename = os.path.join(OUTPUT_DIR, f"RandomForest_feature_list_{var_set_name}.pkl")
    ...
    sample_weights = np.where(y==1, 4.0, 1.0)
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(outer_kf.split(X, y), start=1):
            ...
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_rf(params_list[i]))
            chosen_idx = candidate_idxs_sorted[0]
            chosen_params = params_list[chosen_idx]

            best_model = RandomForestClassifier(random_state=seed, **chosen_params)
    ...
    all_models_sorted = sorted(all_models, key=lambda x:(-x[3], complexity_rf(x[1])))
    final_model = all_models_sorted[0][0]
    model_filename = os.path.join(OUTPUT_DIR, f"RandomForest_{var_set_name}.pkl")
    joblib.dump(final_model, model_filename)
    print(f"Saved best model: {model_filename}")

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-RandomForest.xlsx")
