...
def complexity_l1log(params):
    solver_weight = {"liblinear":1, "saga":2}
...
for var_set_name, var_set in variable_sets.items():
    ...
    joblib.dump(label_encoders, os.path.join(OUTPUT_DIR, f"L1log_label_encoders_{var_set_name}.pkl"))

    feature_list = var_set["catvars"] + var_set["convars"]
    joblib.dump(feature_list, os.path.join(OUTPUT_DIR, f"L1log_feature_list_{var_set_name}.pkl"))
    ...
    joblib.dump(scaler, os.path.join(OUTPUT_DIR, f"L1log_{var_set_name}_scaler.pkl"))
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(outer_kf.split(X_scaled, y), start=1):
            ...
            base_model = LogisticRegression(penalty="l1", max_iter=1000, random_state=seed)
            ...
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_l1log(params_list[i]))
            chosen_idx = candidate_idxs_sorted[0]
            chosen_params = params_list[chosen_idx]

            best_model = LogisticRegression(penalty="l1", max_iter=1000, random_state=seed, **chosen_params)
    ...
    all_models_sorted = sorted(all_models, key=lambda x: (-x[3], complexity_l1log(x[1])))
    final_model = all_models_sorted[0][0]
    joblib.dump(final_model, os.path.join(OUTPUT_DIR, f"L1log_{var_set_name}.pkl"))
    print(f"Saved best model for {var_set_name}")

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-L1Logistic.xlsx")
