...
for var_set_name, var_set in variable_sets.items():
    ...
    le_filename = os.path.join(OUTPUT_DIR, f"Decitree_label_encoders_{var_set_name}.pkl")
    ...
    feat_filename = os.path.join(OUTPUT_DIR, f"Decitree_feature_list_{var_set_name}.pkl")
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(outer_kf.split(X, y), start=1):
            ...
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_dt(params_list[i]))
            chosen_idx = candidate_idxs_sorted[0]
            chosen_params = params_list[chosen_idx]

            best_model = DecisionTreeClassifier(random_state=seed, **chosen_params)
    ...
    all_models_sorted = sorted(all_models, key=lambda x: (-x[3], complexity_dt(x[1])))
    final_model = all_models_sorted[0][0]
    model_filename = os.path.join(OUTPUT_DIR, f"DecisionTree_{var_set_name}.pkl")
    joblib.dump(final_model, model_filename)
    print(f"Saved best model: {model_filename}")

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-DecisionTree.xlsx")
