...
def complexity_etc(params):
    depth = params["max_depth"] if params["max_depth"] is not None else 20
...
for var_set_name, var_set in variable_sets.items():
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X, y), start=1):
            ...
            candidate_idxs_sorted = sorted(candidate_idxs, key=lambda i: complexity_etc(params_list[i]))
            chosen_idx = candidate_idxs_sorted[0]
            chosen_params = params_list[chosen_idx]

            best_model = ExtraTreesClassifier(random_state=seed, **chosen_params)
    ...
    all_models_sorted = sorted(all_models, key=lambda x: (-x[3], complexity_etc(x[1])))
    final_model = all_models_sorted[0][0]

    joblib.dump(final_model, os.path.join(OUTPUT_DIR, f"ExtraTrees_{var_set_name}.pkl"))
    joblib.dump(label_encoders, os.path.join(OUTPUT_DIR, f"ExtraTrees_label_encoders_{var_set_name}.pkl"))
    joblib.dump(var_set["catvars"] + var_set["convars"],
                os.path.join(OUTPUT_DIR, f"ExtraTrees_feature_list_{var_set_name}.pkl"))

output_path = os.path.join(OUTPUT_DIR, "5-fold-CV-ExtraTrees.xlsx")
